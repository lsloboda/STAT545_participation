...
fox %>% 
  str_split("") %>% 
  '[['(1) %>% #this removes the initial list structure; subsets the contents of the first col
  str_to_lower() %>%  #converts string to lowercase letters
  table() 
```
...
fruit %>% head
fruit %>% 
    str_pad(width=7, side="right", pad="$") %>%  #make sure that every words is width 7, if we need more characters, add $ to the right
    head()
```
...
str_c(words[3:4], words[5:8], sep=" & ") #recycling happens (repeats vector until it  matches the length other vectors, if needed)
str_c(words[3:4], words[5:8], sep=" & ", collapse=", ") #collapses into 1 column
```
...
cat(contents[1]) #contents of first
```
...
Here, `.` stands for "any single character". #A single wildcard!
...
What if I wanted to literally search for countries with a period in the name? Escape with `\`, although R requires a double escape. #R parses \\ to \ which is parsed by regex
...
```{r}
str_view_all_match(countries, pattern = "[^aeiou][^aeiou][^aeiou]") #carat indiciated "not" when inside []; but it means something else when outside []!
```
...
```{r}
c("bear", "beer", "bar") %>% 
    str_view_all_match(pattern = "be(e|a)r") #| is or
```
...
```{r}
str_view_all_match(countries, "ro+") #at least one o
```
...
```{r}
str_view_all_match(countries, pattern = "land$") #$ means the end of the string
str_view_all_match(countries, pattern = "$") #this finds the end of each line
```
...
```{r}
str_view_all_match(countries, "^[^aeiouAEIOU]*$") #no results since theres no country that doesnt contain entirely no vowels, when ^ is outside [], it means "start" of string
# the asterisk means "zero or more" of the thing before it
```
...
```{r}
str_view_all(c("abad", "abbd"), pattern="(a)(b)\\1") #a, b, then whatever was resolved in group 1
str_view_all(c("abad", "abbd"), pattern="(a)(b)\\2") #similarly, whatever the second group was resolved to
```
...
```{r}
str_view_all(c("bananas", "Who can? Bananas can."), "(.)(.)\\1\\2.*\\1\\2") #search for anything followed by anything, once its resolved, repeat the first group, repeat the second group, then any number of characters followed by repeats
```
