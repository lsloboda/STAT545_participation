...
do.call(bind_rows, lotr_list) #applies bind_rows to each element of lotr_list, without knowing how many elements there are
```
...
all <- experiment %>% expand(nesting(name, trt), rep) #nesting looks at combinations with repetition (i.e. combinations that exist)
all
```
...
`uncount` (as the opposite of `dplyr::count()`) #makes a repeated row for the number of obsevations that you have
