...
my_commute <- 46
my_commute #or print(my_commute)
...
# Note that the file doesn't run top to bottom... to do this, click Source > Source

## Let's make a vector of commute times, stored in the variable `commute`.
## 1. Start with your commute time, using the `c` function. No magic numbers! (hardcoding)
## 2. Using commas, add at least 2 more times from those around you.
commute <- c(my_commute, 25, 35, 15, 50) #adding a character to the list changes the variable to a character vector
commute
class(commute)

## Use the `length` function to store the number of observations in `n`.
(n <- length(commute)) #adding parantheses shows you the output
...
(diffs <- commute - mu) #note that R loops the subtraction along every value of the vector automatically
...
var(commute) == variance #TRUE is not a character, it's a logical
typeof(TRUE)
