...
#unclass(my_lm) #shows the underlying list structure (the text output above isn't really the object) - but it's really long!
names(my_lm) #pick out what you want to extract
my_lm$coefficients
```
...
```{r}
predict(my_lm) %>% #takes the data frame, for each row, takes the x value and predicts a y value; outputs a vector of predictions for each row (only works for tidy data; cant have y values spread out over multiple columns)
  head() #only apply to first 6 columns

predict(my_lm, newdata = tibble(gdpPercap = c(500,600,700))) #newdata = what x values do you want to make predictions on; must be in the form of a data frame
gapminder %>% head()

#takeaway on lm: input is easy, output is messy; broom package helps with cleaning up the output
```
...
gof <- glance(my_lm)
gof$p.value #doesnt keep precision
