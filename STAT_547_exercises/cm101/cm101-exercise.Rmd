...
square2 <- function(x) {
  y <- x^2
  y #R will execute the last line of the function and return it
}
square2(10)

square3 <- function(x) {
  y <- x^2 #useful if you want to stop the function early
  return(y)
  5
}
square3(10)
...
```{r}
max_minus_min(1:10)
max_minus_min(runif(100)) # generate 100 random numbers
#but its annoying to manually check all your functionsl so we will use the "test that" package
```

Let's formalize this testing with the `testthat` package. `expect_*()` functions:

```{r}
expect_equal(0.1 + 0.2, 0.3) #if test passes, nothing is returned
expect_identical(0.1 + 0.2, 0.3) #much stricter (likely a floating point error)
```
...
```{r}
max_minus_min(gapminder[c('lifeExp', 'gdpPercap', 'pop')]) #subsetting to numeric variables
max_minus_min(c(TRUE, TRUE, FALSE, TRUE, TRUE)) #coerces these to zeros and ones
```
...
#0.9 quantile means that every time you draw a number, 90% of the time, our number will be less than the output
```
...
qd2(rnorm(100), probs=c(0.25, 0.57))
```
...
#ellipses (...) is R's way of waying that you can put as many arguments as you want
#using it twice means that whatever goes into the first one will go into the second one (note: they will have to be named)
```