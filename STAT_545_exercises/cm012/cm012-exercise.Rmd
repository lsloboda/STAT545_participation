...
iris %>% 
  ...
  str() #see structure; R is showing us the number underneath the string
```

```{r}
c(iris$Species, "setosa") #only shows the character part of the vector at the end!
#takeaway: if you get odd results and you think you have a character, check the type (it may be a factor); R tends to automatically coerce characters into factors (ggplot2 does this, for example)
as.character(iris$Species) #now we see the character
```


- Base R way of interacting with factors:
    - `factor()`, or `forcats::parse_factor()`.
    - `levels()`#what are the levels
...
nlevels(draw)
fct_count(draw) #from the forcats package; gives us the observations and the count of each
```
...
fct_c(lotr1$Film, lotr2$Film) #auto-added both levels together
```
...
```{r}
rbind(lotr1, lotr2) %>% str() #assumes you want to add a column; more forgiving
#bind_rows(lotr1, lotr2) %>% str() #does not make any assumptions

#coercing, basically same as converting (some small diference)
as.character(5)
```
...
ggplot(gap_small, aes(continent))+
  geom_bar()+
  scale_x_discrete(drop=FALSE) #keep factor levels that don't have entries
```
...
gap_small %>% 
  mutate(continent = fct_drop(continent)) %>%  #usually fct functions apply to factor itself, not the data frame
  str() #continent now only has the 3 levels presentin the data frame
```
...
qplot(cont) #automatically makes a bar chart
```
...
```{r}
gapminder %>% 
  mutate(continent = fct_reorder(continent, lifeExp)) %>% #reoder is similar to a group_by and summarize
...
gap_asia_2007$country %>% 
  fct_relevel("Syria") %>% #make it first
  levels() %>% 
  head()

gap_asia_2007$country %>% 
  fct_relevel("Syria", "Sweden", after=2) %>% #make it second, then add Sweden
  levels() %>% 
  head()
```
...
```{r}
cont %>% 
  fct_lump() %>% #default: ensures that the other category is the smallest (while combining as many as possible into it)
  qplot()

cont %>% 
  fct_lump(n=2) %>%  #keep the top 2, lump everything else together into one
  qplot()
```
