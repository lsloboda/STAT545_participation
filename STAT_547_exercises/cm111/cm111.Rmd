...
```{r}
simple_page %>% html_structure() #shows an abbereviated version
```

3. Apply `html_text()` to `simple_page`: gives me all the text (not the tags).

```{r}
simple_page %>% html_text() %>% cat() #looks at entire HTML code as a single unit; cat will render it without breaks
```
...
```{r}
simple_page %>% #paragraph tags
	html_nodes("p") %>% 
  html_text() #pulls the text that is between the p tags
simple_page %>% 
	html_nodes("a") %>% #hyperlink tags
  html_text()
simple_page %>% 
	html_nodes("#wiki") %>% #can manually specify the tags
  html_text()
```
