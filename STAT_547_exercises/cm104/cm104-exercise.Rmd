...
#recall, ~ is the same as functio(.x)
...
map2_chr(name, commute, comm_fun) #without chr, map2 will output a list
...
```{r}
#difference here is defining the function internally vs outside
...
#got_chars[[1]]
got_alias <- got_chars %>% {
  tibble(name = map_chr(., "name"), #subset
         aliases = map(., "aliases")) 
}
...
```{r}
got_alias %>%
  {setNames(.$aliases, .$name)} %>%   #the dot will refer to the entire aliases dataframe
  `[[`("Melisandre") #function of the double brackets is the same as surrounding the thing in the brackets, same as x[["Melisandre"]]
```
...
```{r}
icecream %>% 
  filter(map_lgl(flav, ~ any(.x == "strawberry"))) #.x is a vector of ice cream flavours and we want any one of the entries to be equal to strawberry
```
