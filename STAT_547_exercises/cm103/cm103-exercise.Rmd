...
```{r}
x <- 1:10
sqrt(x) #already vectorized, so this isn't really necessary
map(x, sqrt) #map always outputs a list
```
...
#method 2
map_chr(wesanderson, head, n=1) #only take the first elements
#map_dbl(wesanderson, head, n=1) #wont work; cant coerce char vectors to double
#method 3
...
```{r}
map_chr(wesanderson, 1) # %>% unname() (if you dont want the names to show up)
```
...
} #reason for using curly brackets -> pipe operator pipes output into the next function; this allows us to use the '.' to substitute the object to the left of the pipe
#HIGHLY recommend to read help section on curly brackets vs round brackets
```
